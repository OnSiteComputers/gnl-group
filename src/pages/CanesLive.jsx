...
const PLAYOFF_SERIES_URL = "https://api-web.nhle.com/v1/playoff-series/carousel/20252026/";

function findCanesSeries(playoffData) {
  const rounds = Array.isArray(playoffData?.rounds) ? playoffData.rounds : [];
  const allSeries = rounds.flatMap((round) =>
  ...
  const canesSeries = allSeries.filter(
    (series) =>
      series?.topSeed?.abbrev === CANES_ABBREV ||
      series?.bottomSeed?.abbrev === CANES_ABBREV
  );

  if (!canesSeries.length) return null;

  const currentRound = Number(playoffData?.currentRound || 0);
  return (
    canesSeries.find((series) => Number(series.roundNumber) === currentRound) ||
    canesSeries[canesSeries.length - 1]
  );
}

function normalizeCanesSeries(series) {
  if (!series) return null;

  const top = series.topSeed || {};
  const bottom = series.bottomSeed || {};
  const canesSeed = top.abbrev === CANES_ABBREV ? top : bottom;
  const oppSeed = top.abbrev === CANES_ABBREV ? bottom : top;

  return {
    roundLabel: series.roundAbbrev || series.seriesLabel || "Series",
    canes: Number(canesSeed.wins ?? 0),
    opp: Number(oppSeed.wins ?? 0),
    oppName: oppSeed.abbrev || "OPP",
    neededToWin: Number(series.neededToWin || 4),
  };
}
...
export default function CanesLive() {
  ...
  const fetchSeries = useCallback(async () => {
    try {
      const res = await fetch(PLAYOFF_SERIES_URL, { cache: "no-store" });
      if (!res.ok) throw new Error(`Series feed returned ${res.status}`);

      const playoffData = await res.json();
      const canesSeries = normalizeCanesSeries(findCanesSeries(playoffData));

      if (canesSeries) {
        setSeriesFeed(canesSeries);
        setSeriesErr(null);
      } else {
        setSeriesErr("No Canes playoff series found in the NHL series feed.");
      }
    } catch {
      setSeriesErr("Couldn't reach the NHL series feed.");
    }
  }, []);
        ...
        <section style={styles.grid}>
          <div style={styles.card}>
            <div style={styles.cardTitle}>
              {seriesFeed?.roundLabel ? `${seriesFeed.roundLabel} · BEST OF ${seriesFeed.neededToWin * 2 - 1}` : "SERIES · BEST OF 7"}
            </div>
            {seriesFeed ? (
              <>
                ...
                <div style={styles.seriesNote}>Series score from NHL playoff series feed.</div>
              </>
            ) : (
