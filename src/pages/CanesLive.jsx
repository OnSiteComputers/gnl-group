...
export default function CanesLive() {
  ...
  const fetchLive = useCallback(async () => {
    ...
    try {
      ...
      // Fallback only. The NHL schedule feed is the source of truth because
      // the old Worker can return a stale game, such as an old Montreal score.
      if (!WORKER_URL) {
        setErr("Couldn't reach the official NHL schedule feed.");
        setLoading(false);
        return;
      }
      ...
      try {
        ...
        if (data?.found) {
          ...
          setLastUpdated(new Date());
          setErr("Official NHL feed was unreachable, so this is using the backup feed.");
        } else {
          setErr("No Canes game found in the official or backup feed.");
        }
      } catch {
        setErr("Couldn't reach the official NHL schedule feed or the backup live feed.");
      }
    } finally {
      setLoading(false);
    }
      ...
      <main style={styles.shell}>
        ...
        <section style={styles.controls}>
          ...
          <div style={styles.liveTag}>
            ...
            {liveFeed ? "OFFICIAL NHL FEED · auto every 15s" : "MANUAL MODE"}
          </div>
